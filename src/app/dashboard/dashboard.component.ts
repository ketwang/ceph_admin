...
export class DashboardComponent implements OnInit {
  ...
  public cephSummaryDetail: Object  = {
    cluster: "unknown",
    total: 0,
    used: 0,
    objectsNum: 0,
  ...
  loadCephInfo(id: string) {
    this.cephClusterDetailBusy = this.http.get('http://localhost:8000/cluster/CapacListLast/'+id).subscribe(
      res => {this.cephSummaryDetail = res.json();console.log(this.cephSummaryDetail)}
    );
  }
  loadCephPoolInfo(id: string) {
    this.cephClusterPoolDetailBusy = this.http.get('http://localhost:8000/pool/CapacListLast/'+id).subscribe(
      res => {this.cephPoolSummary = res.json();console.log(this.cephPoolSummary)}
    );
  }
  loadCephOSDInfo(id: string) {
    this.cephClusterOSDDetailBusy = this.http.get('http://localhost:8000/osd/OSDListLast/'+id).subscribe(
      res => {this.cephOSDSummary = res.json();console.log(this.cephOSDSummary)}
    );
  }
